...
GrooveCat.PERONALITIES = {"Lively", "Bipolar", "Sleepy", "Ninja"} -- Houdini, coming soon
...
function GrooveCat:get_serialized()
    ...
    d.autoShuffle = self.autoShuffle
...
function GrooveCat:load_serialized(data)
    ...
    self.autoShuffle = data.autoShuffle
end
...
function GrooveCat:purr_loop()
    ...
    while self.purring do
        ...
        if self.enabled then
            if self.bounce_seq.pos == 1 and self.autoShuffle then
                self:shuffle_loop()
            end
            ...
            elseif self.personality == 4 then -- ninja, doesn't actually launch anything, triggers notes in order of sequence and probability
                self:ninja_meow()
            end
        end
    end
...
function GrooveCat:ninja_meow()
    self.last_note_fired = false
    
    if math.random(100) <= self.probability then
        local note = self.bounce_seq.data[self.bounce_seq.pos]
        if self.onMeow ~= nil then self.onMeow(self, note) end
        
        if note > 0 then self.last_note_fired = true end
    end
...
function GrooveCat:draw()
    ...
    elseif self.personality == 4 then
        if self.last_note_fired then 
            if self.bounce_seq.pos % 2 == 0 then
                img = (grooveCatImg.."grooveCat2")
            else 
                img = (grooveCatImg.."sleepyCatAwake")
            end
        else
            img = (grooveCatImg.."sleepyCat")
        end
